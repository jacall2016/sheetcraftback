...
def extract_file_info(file):
    if not hasattr(file, 'name'):
        file.name = 'uploaded_live_file.xlsx'
...
def process_excel_file(file):
    ...
    df.loc[df['Plate_Well'].str.contains(r'23$|24$|^(?:A|B|C|D|E|F|G|H|I|J|K|L|M|N|O|P)(1|2)$'), 'High_Controls_z_score'] = df['live_z_score']
    df = df[[
        'Library', 'Library_Copy_number', 'Plate_Name', 'Plate_Well', 'Molecule_Name', 'SMILES', 
        'Compound_number', 'live_z_score', 'hits_live_z_score', 'High_Controls_z_score'
    ]]
    df.rename(columns={
        'live_z_score': 'All_Compounds_live_z_score',
        'hits_live_z_score': 'Hits_live_z_score'
    }, inplace=True)
    return df
...
def add_missing_wells(df):
    ...
    for plate_name, group in df.groupby('Plate_Name'):
        ...
        for missing_well in missing_wells:
            new_row = {
                'Library': group['Library'].iloc[0],  # Take the first value, assuming all are the same for a plate
                'Library_Copy_number': group['Library_Copy_number'].iloc[0],  # Same as above
                'Plate_Name': plate_name,
                'Plate_Well': missing_well,
                'Molecule_Name': "",
                'SMILES': "",
                'Compound_number': "",
                'All_Compounds_live_z_score': np.nan,
                'Hits_live_z_score': np.nan,
...
def compile_live(files):
    all_data = pd.DataFrame(columns=[
        'Library', 'Library_Copy_number', 'Plate_Name', 'Plate_Well', 'Molecule_Name', 'SMILES', 
        'Compound_number', 'All_Compounds_live_z_score', 'Hits_live_z_score', 'High_Controls_z_score'
    ])
    for file in files:
    ...
    return processed_data, output, 'Compiled_live.xlsx'
