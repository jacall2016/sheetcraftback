...
def extract_file_info(file):
    if not hasattr(file, 'name'):
        file.name = 'uploaded_phl_file.xlsx'
...
def process_excel_file(file):
    ...
    df.loc[df['Plate_Well'].str.contains(r'23$|24$|^(?:A|B|C|D|E|F|G|H|I|J|K|L|M|N|O|P)(1|2)$'), 'High_Controls_z_score'] = df['phl_z_score']
    df = df[[
        'Library', 'Library_Copy_number', 'Plate_Name', 'Plate_Well', 'Molecule_Name', 'SMILES', 
        'Compound_number', 'phl_z_score', 'hits_phl_z_score', 'High_Controls_z_score'
    ]]
    df.rename(columns={
        'phl_z_score': 'All_Compounds_phl_z_score',
        'hits_phl_z_score': 'Hits_phl_z_score'
    }, inplace=True)
    return df
    ...
    for plate_name, group in df.groupby('Plate_Name'):
        ...
        for missing_well in missing_wells:
            new_row = {
                ...
                'SMILES': "",
                'Compound_number': "",
                'All_Compounds_phl_z_score': np.nan,
                'Hits_phl_z_score': np.nan,
...
def compile_phl(files):
    all_data = pd.DataFrame(columns=[
        'Library', 'Library_Copy_number', 'Plate_Name', 'Plate_Well', 'Molecule_Name', 'SMILES', 
        'Compound_number', 'All_Compounds_phl_z_score', 'Hits_phl_z_score', 'High_Controls_z_score'
    ])
    for file in files:
    ...
    return processed_data, output, 'Compiled_phl.xlsx'
