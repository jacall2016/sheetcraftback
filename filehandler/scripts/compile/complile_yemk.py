...
def process_excel_file(file):
    ...
    df.loc[df['Plate_Well'].str.contains(r'23$|24$|^(?:A|B|C|D|E|F|G|H|I|J|K|L|M|N|O|P)(1|2)$'), 'High_Controls_z_score'] = df['yemk_z_score']
    df = df[[
        'Library', 'Library_Copy_number', 'Plate_Name', 'Plate_Well', 'Molecule_Name', 'SMILES', 
        'Compound_number', 'yemk_z_score', 'hits_yemk_z_score', 'High_Controls_z_score'
    ]]
    df.rename(columns={
        'yemk_z_score': 'All_Compounds_YEMK_z_score',
        'hits_yemk_z_score': 'Hits_YEMK_z_score'
    }, inplace=True)
    return df
    ...
    for plate_name, group in df.groupby('Plate_Name'):
        ...
        for missing_well in missing_wells:
            new_row = {
                ...
                'SMILES': "",
                'Compound_number': "",
                'All_Compounds_YEMK_z_score': np.nan,
                'Hits_YEMK_z_score': np.nan,
...
def compile_yemk(files):
    all_data = pd.DataFrame(columns=[
        'Library', 'Library_Copy_number', 'Plate_Name', 'Plate_Well', 'Molecule_Name', 'SMILES', 
        'Compound_number', 'All_Compounds_YEMK_z_score', 'Hits_YEMK_z_score', 'High_Controls_z_score'
    ])
    for file in files:
    ...
    return processed_data, output, 'Compiled_YEMK.xlsx'
