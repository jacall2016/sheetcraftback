...
def process_excel_file(file):
    plate_name, library, library_copy_number = extract_file_info(file.name)
...
def compile_flip700(files):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        final_sorted_data.to_excel(writer, index=False, sheet_name='Compiled Data')
    output.seek(0)
    
    return final_sorted_data, output, 'compiled_flip700.xlsx'
