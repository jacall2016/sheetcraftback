...
class FileFieldFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            # Encode the file content and save it in the session
            ...
            if 'file_content' in self.request.session and 'file_name' in self.request.session:
                logger.debug(f"Session Data Set: {self.request.session.get('file_content')}, {self.request.session.get('file_name')}")
            else:
...
def download_file(request):
    ...
    if encoded_file_content and file_name:
        try:
            ...
            return HttpResponse("Error retrieving the file.", status=500)
    else:
        logger.warning("No file content or file name in session.")
